    ...
    <HTMLContainer className="group/image overflow-visible">
      ...
      {isSelected ? (
        <Button
          ...
          variant="outline"
          size="sm"
          className="pointer-events-auto absolute top-full left-1/2 mt-3 -translate-x-1/2 rounded-full bg-background shadow-md"
          aria-label="以这张图片为参考继续创作"
          title="以这张图片为参考继续创作"
          onPointerDown={stopPointer}
          onClick={() => action("continue-from-image")}
        >
          <Sparkles />
          继续创作
        </Button>
